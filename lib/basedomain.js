...
function URI(/**String*/ spec)
{
  ...
  {
    var queryChar = spec.indexOf("?", this._hostPortStart);
    var fragmentChar = spec.indexOf("#", this._hostPortStart);
    if (queryChar >= 0 && fragmentChar >= 0)
      this._hostPortEnd = Math.min(queryChar, fragmentChar);
    else if (queryChar >= 0)
      this._hostPortEnd = queryChar;
    else if (fragmentChar >= 0)
      this._hostPortEnd = fragmentChar;
    else
